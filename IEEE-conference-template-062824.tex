...
\author{
	...
	\IEEEauthorblockN{Peng-Wen, Chen}
	\IEEEauthorblockA{
		\textit{Department of Mechanical Engineering} \\
		\textit{National Central University}\\
		Taoyuan, Taiwan \\
	...
	\IEEEauthorblockN{Ting-You, Chen}
	\IEEEauthorblockA{
		\textit{Department of Mechanical Engineering} \\
		\textit{National Central University}\\
		Taoyuan, Taiwan \\
	...
	\IEEEauthorblockN{Hsiang-Chieh, Chen}
	\IEEEauthorblockA{
		\textit{Department of Mechanical Engineering} \\
		\textit{National Central University}\\
		Taoyuan, Taiwan \\
...
\begin{figure}
    \centering
    \includegraphics[width = 5 cm]{fig_1.jpg}
...
The right-wheel controller incorporates the left-wheel output and the Hall-recorded speed difference within a fixed interval ("Hall Signal Difference" in table \ref{tab:fuzzy_rules}) to estimate and adjust the right-wheel speed difference compare to the left-wheel for synchronization. The resulting speed differences are converted into PWM adjustments for the motor drivers, with reverse signals omitted to ensure user safety.
It's fuzzy control logic can be summrize by table \ref{tab:fuzzy_rules}.

In table \ref{tab:fuzzy_rules}, the abbreviation meaning is as following.
NB, negative big value ;
NS, negative small value ;
Z, value close to zero ;
PS, positive small value ;
PB, positive big value ;

\begin{table}[h]
\centering

\caption{Fuzzy logic for right wheel}
\label{tab:fuzzy_rules}

\begin{tabular}{cc|c|c|c|c|c|}
\cline{3-7} % Draws a horizontal line starting from column 3 to 7
 & & \multicolumn{5}{c|}{Hall Signal Difference} \\ \cline{3-7} 
 & & NB & NS & Z & PS & PB \\ \hline
\multicolumn{1}{|c|}{\multirow{5}{*}{\shortstack{Left\\Control\\Value}}} & NB & 0 & -5 & -10 & -15 & -20 \\ \cline{2-7} 
\multicolumn{1}{|c|}{} & NS & +5 & 0 & -5 & -10 & -15 \\ \cline{2-7} 
\multicolumn{1}{|c|}{} & Z & +10 & +5 & 0 & -5 & -10 \\ \cline{2-7} 
\multicolumn{1}{|c|}{} & PS & +15 & +10 & +5 & 0 & -5 \\ \cline{2-7} 
\multicolumn{1}{|c|}{} & PB & +20 & +15 & +10 & +5 & 0 \\ \hline
\end{tabular}
\end{table}
...
For uphill, the rollator did provide sufficient propulsion through PWM to assist the user, which assit them pass a more challenging terrian.
As for downhill, power-off braking providing sustained braking torque and ensuring operational safety.

\section{Conclusion and Future Work}
In conclusion, this design assists users across various outdoor use cases. The primary goal is to support the elderly individuals, encouraging physical activity and ultimately enhancing their quality of life.

Future research will focus on further extend this design to support specialized populations, such as patients with congenital skeletal dysplasia or those suffering from muscular atrophy.
